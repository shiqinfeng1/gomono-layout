...
function redis::install()
{
  ...
  common::sudo "sudo ufw disable"
  common::sudo "sudo ufw status"

  # 4. 启动 Redis
  common::sudo "redis-server /etc/redis/redis.conf"

  redis::status || return 1
  redis::info
  log::info "install Redis successfully"
}
...
function redis::uninstall()
{
  ...
  set -o errexit
  log::info "uninstall Redis successfully"
}
...
function redis::status()
{
  if [[ -z "`pgrep redis-server`" ]];then
    log::error_exit "Redis not running, maybe not installed properly"
    return 1
  fi


  redis-cli --no-auth-warning -h ${REDIS_HOST} -p ${REDIS_PORT} -a "${REDIS_PASSWORD}" --hotkeys || {
    log::error "can not login with ${REDIS_USERNAME}, redis maybe not initialized properly"
    return 1
  }

  log::info "redis-server status active"
}
